...
# ---- Conf hardening helpers (#260) ----

_cc_install_security_log() {
    local level="$1" event="$2" detail="$3"
    local logfile="${PROJECT_DIR}/.claude/cognitive-core/security.log"
    local ts
    ts=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
    mkdir -p "$(dirname "$logfile")" 2>/dev/null || true
    echo "${ts} [${level}] ${event}: ${detail}" >> "$logfile" 2>/dev/null || true
}
...
_cc_verify_conf_owner() {
    local conf="$1"
    local expected file_uid
    expected=$(id -u)
    file_uid=$(_cc_conf_owner_uid "$conf")
    if [ -z "$file_uid" ]; then
        err "Conf owner check failed: could not stat ${conf}"
        return 1
    fi
    if [ "$file_uid" != "$expected" ]; then
        if [ "${CC_CONF_OWNER_SKIP:-0}" = "1" ]; then
            _cc_install_security_log "WARN" "owner-check-skipped" \
                "conf=${conf} file_uid=${file_uid} expected=${expected}"
            warn "Conf owner mismatch (file_uid=${file_uid}, expected=${expected}) — CC_CONF_OWNER_SKIP=1 honored"
            return 0
        fi
        err "Conf owner mismatch: ${conf} is owned by uid ${file_uid} (expected ${expected})"
        err "Set CC_CONF_OWNER_SKIP=1 to bypass (CI/Docker)"
        return 1
    fi
    return 0
}

_cc_check_framework_root_match() {
    local conf="$1"
    [ -n "${CC_FRAMEWORK_ROOT:-}" ] || return 0
    local current
    current="$(realpath "${SCRIPT_DIR}" 2>/dev/null || (cd "${SCRIPT_DIR}" && pwd))"
    if [ "$CC_FRAMEWORK_ROOT" != "$current" ]; then
        err "Framework root mismatch in ${conf}"
        err "  conf says:  CC_FRAMEWORK_ROOT=${CC_FRAMEWORK_ROOT}"
        err "  install is: ${current}"
        err "Refusing to overwrite. Remove CC_FRAMEWORK_ROOT from the conf, or re-install from the original path."
        return 1
    fi
    return 0
}
...
if [ -f "$CONF_FILE" ]; then
    ...
    source "$CONF_FILE"
    _cc_check_framework_root_match "$CONF_FILE" || exit 1
...
3. Run lint before every commit
    ...
    CC_REQUIRE_SHARED_STATE_APPROVAL="true"
    ...
    # Framework root resolved with realpath → cd+pwd fallback (no python3).
    _CC_FRAMEWORK_ROOT_VAL="$(realpath "${SCRIPT_DIR}" 2>/dev/null || (cd "${SCRIPT_DIR}" && pwd))"
...
# ===== FRAMEWORK ANCHOR =====
# Pinned at install time. Hooks/utilities anchor here for the framework source.
# Do not edit manually. See mindcockpit-ai/cognitive-core#260.
CC_FRAMEWORK_ROOT="${_CC_FRAMEWORK_ROOT_VAL}"
...
CC_REQUIRE_SHARED_STATE_APPROVAL="${CC_REQUIRE_SHARED_STATE_APPROVAL}"
CC_JIRA_ALLOWED_TRANSITIONS="${CC_JIRA_ALLOWED_TRANSITIONS}"
CC_ORPHAN_AUTO_KILL="${CC_ORPHAN_AUTO_KILL}"
...
# ---- Conf hardening: chmod 0444 + owner verify (#260) ----
# Lock conf to read-only after write. Applies to both freshly-written confs
# and pre-existing confs on --force reinstalls.
if [ -f "$CONF_FILE" ]; then
    chmod 0444 "$CONF_FILE" 2>/dev/null || true
    _cc_verify_conf_owner "$CONF_FILE" || exit 1
fi
