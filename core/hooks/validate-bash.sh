...
# Strip quoted strings and heredoc bodies so patterns only match actual commands,
# not content inside commit messages, echo strings, or comments.
...
if echo "$CMD_STRIPPED" | grep -qE 'rm[[:space:]]+(-[a-z]*f[a-z]*[[:space:]]+)?(/|/etc|/usr|/var|/home|/System|/Library)([[:space:]]|$)'; then
    REASON="Blocked: rm targeting system-critical path"
fi

# git push --force to main/master
if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'git[[:space:]]+push[[:space:]]+.*--force.*[[:space:]]+(master|main)([[:space:]]|$)|git[[:space:]]+push[[:space:]]+.*-f[[:space:]]+.*(master|main)([[:space:]]|$)'; then
    REASON="Blocked: force push to ${CC_MAIN_BRANCH:-main}"
fi

# git reset --hard
if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'git[[:space:]]+reset[[:space:]]+--hard'; then
    REASON="Blocked: git reset --hard (destructive, may lose work)"
fi

# DROP TABLE / TRUNCATE TABLE
if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qiE '(drop|truncate)[[:space:]]+table'; then
    REASON="Blocked: DROP/TRUNCATE TABLE (destructive database operation)"
fi

# DELETE FROM without WHERE
if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qiE 'delete[[:space:]]+from[[:space:]]+[a-zA-Z0-9_]+[[:space:]]*$|delete[[:space:]]+from[[:space:]]+[a-zA-Z0-9_]+[[:space:]]*;'; then
    REASON="Blocked: DELETE FROM without WHERE clause (would delete all rows). Add a WHERE clause to limit scope"
fi

# rm .git
if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'rm[[:space:]]+(-[a-z]*[[:space:]]+)?\.git([[:space:]]|$|/)'; then
    REASON="Blocked: removing .git directory"
fi

# chmod 777
if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'chmod[[:space:]]+777'; then
...
if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'git[[:space:]]+clean[[:space:]]+-[a-z]*f' && ! echo "$CMD_STRIPPED" | grep -qE 'git[[:space:]]+clean[[:space:]]+-[a-z]*n'; then
...
if [ "$_SECURITY_LEVEL" != "minimal" ]; then
    ...
    if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'curl[[:space:]]+.*-d[[:space:]]+.*@'; then
        REASON="Blocked: potential data exfiltration (curl -d @file)"
    fi
    if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'cat[[:space:]]+.*\|.*curl'; then
        REASON="Blocked: potential data exfiltration (cat | curl)"
    fi
    if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'cat[[:space:]]+.*\|.*([[:space:]]|^)nc([[:space:]]|$)'; then
        REASON="Blocked: potential data exfiltration (cat | nc)"
    fi
    if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE '(^|[[:space:]])env[[:space:]]*\|'; then
        REASON="Blocked: environment variable leak (env |)"
    fi

    # Encoded command bypass
    if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'base64.*-d.*\|.*(ba)?sh'; then
        REASON="Blocked: encoded command execution (base64 -d | sh)"
    fi
    if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'echo[[:space:]]+.*\|.*base64.*-d'; then
        REASON="Blocked: encoded command execution (echo | base64 -d)"
    fi
    if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE '(^|[[:space:]])eval[[:space:]]+.*\$\('; then
        REASON="Blocked: eval with command substitution"
    fi

    # Pipe-to-shell (supply chain attack vector)
    if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'curl[[:space:]]+.*\|.*(ba)?sh'; then
        REASON="Blocked: pipe-to-shell (curl | sh) — supply chain risk"
    fi
    if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'wget[[:space:]]+.*\|.*(ba)?sh'; then
        REASON="Blocked: pipe-to-shell (wget | sh) — supply chain risk"
    fi
    if [ -z "$REASON" ] && echo "$CMD_STRIPPED" | grep -qE 'wget[[:space:]]+.*-O-[[:space:]]*\|'; then
...
if [ -z "$REASON" ] && [ -n "${CC_BLOCKED_PATTERNS:-}" ]; then
    for pattern in $CC_BLOCKED_PATTERNS; do
        if echo "$CMD_STRIPPED" | grep -qE "$pattern"; then
